...
def key_metric_summary(request):
    ...
    # Retrieve customer satisfaction score from the Rating model and handle None
    customer_satisfaction_score = Rating.objects.all().aggregate(
        Avg('score'))['score__avg'] or 0  # Set default value to 0 if it's None

    # Get the last 3 lowest and highest 2 rated customer care representatives.
    if customer_satisfaction_score is not None:
        ...
        # Handle None value, for example, by setting an empty queryset
        lowest_rated_representatives = Agent.objects.none()
        highest_rated_representatives = Agent.objects.none()
...
def create_article(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            article.save()
            return redirect('customer/article_detail', article_id=article.id)
    else:
        form = ArticleForm()
